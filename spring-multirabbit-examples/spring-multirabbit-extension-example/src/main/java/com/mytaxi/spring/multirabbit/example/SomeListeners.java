...
class SomeListeners
{
    ...
    /**
     * First listener, listening to the default Rabbit context (connection provided from one of the configurations of 'spring.multirabbitmq' and defined as 'defaultConnection=true' or from
     * the default 'spring.rabbitmq' configuration.
     *
    ...
    /**
     * Second listener, listening to the specific context 'connectionNameA', which is provided from the configuration 'spring.multirabbitmq.connectionNameA'.
     *
     * @param message the message received.
     */
    ...
    /**
     * Third listener, listening to the specific context 'connectionNameB', which is provided from the configuration 'spring.multirabbitmq.connectionNameB'.
     *
     * @param message the message received.
     */
    ...
    public void listenConnectionNameB(String message)
    {
     ...
     * @param message the message received.
     */
    @RabbitListener(containerFactory = "extendedConnectionNameA",
        bindings = @QueueBinding(
            value = @Queue(SAMPLE_EXT_QUEUE_A),
            exchange = @Exchange(SAMPLE_EXT_EXCHANGE_A),
            key = SAMPLE_EXT_ROUTING_KEY_A))
    public void listenExtendedConnectionNameA(String message)
    {
     ...
     * @param message the message received.
     */
    @RabbitListener(containerFactory = "extendedConnectionNameB",
        bindings = @QueueBinding(
            value = @Queue(SAMPLE_EXT_QUEUE_B),
            exchange = @Exchange(SAMPLE_EXT_EXCHANGE_B),
            key = SAMPLE_EXT_ROUTING_KEY_B))
    public void listenExtendedConnectionNameB(String message)
    {
