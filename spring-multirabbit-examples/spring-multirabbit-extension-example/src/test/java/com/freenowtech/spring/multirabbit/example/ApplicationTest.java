...
import static com.freenowtech.spring.multirabbit.example.ExtendedConfiguration.EXTENDED_CONNECTION_A;
import static com.freenowtech.spring.multirabbit.example.ExtendedConfiguration.EXTENDED_CONNECTION_B;
...
public class ApplicationTest {
    ...
    public void shouldResolveContainerFactoryBeans() {
        final List<String> beans
                = Arrays.asList(applicationContext.getBeanNamesForType(SimpleRabbitListenerContainerFactory.class));
        assertTrue(beans.containsAll(Arrays.asList("rabbitListenerContainerFactory", EXTENDED_CONNECTION_A,
                EXTENDED_CONNECTION_B)));
        beans.forEach(bean ->
                assertNotNull(applicationContext.getBean(bean, SimpleRabbitListenerContainerFactory.class)));
    }
