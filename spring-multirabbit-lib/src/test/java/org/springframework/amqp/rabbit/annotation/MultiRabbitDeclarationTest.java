...
public class MultiRabbitDeclarationTest {
    ...
    void shouldEnsureDeclaredQueues() {
        ...
        this.contextRunner
                ...
                .withPropertyValues("spring.multirabbitmq." + broker1 + ".port=" + rabbit1Port)
                .withPropertyValues("spring.multirabbitmq." + broker2 + ".port=" + rabbit2Port)
                .withConfiguration(UserConfigurations.of(ThreeListenersBeans.class))
                .run((context) -> {
                    final RabbitAdmin amqpAdmin = context.getBean(DEFAULT_RABBIT_ADMIN_BEAN_NAME, RabbitAdmin.class);
                    assertThat(amqpAdmin.getQueueProperties(ThreeListenersBeans.QUEUE_0)).isNotNull();
                    assertThat(amqpAdmin.getQueueProperties(ThreeListenersBeans.QUEUE_1)).isNull();
                    assertThat(amqpAdmin.getQueueProperties(ThreeListenersBeans.QUEUE_2)).isNull();

                    final RabbitAdmin rabbitAdmin1 = context.getBean(
                            ThreeListenersBeans.BROKER_NAME_1 + RABBIT_ADMIN_SUFFIX, RabbitAdmin.class);
                    assertThat(rabbitAdmin1.getQueueProperties(ThreeListenersBeans.QUEUE_0)).isNull();
                    assertThat(rabbitAdmin1.getQueueProperties(ThreeListenersBeans.QUEUE_1)).isNotNull();
                    assertThat(rabbitAdmin1.getQueueProperties(ThreeListenersBeans.QUEUE_2)).isNull();

                    final RabbitAdmin rabbitAdmin2 = context.getBean(
                            ThreeListenersBeans.BROKER_NAME_2 + RABBIT_ADMIN_SUFFIX, RabbitAdmin.class);
                    assertThat(rabbitAdmin2.getQueueProperties(ThreeListenersBeans.QUEUE_0)).isNull();
                    assertThat(rabbitAdmin2.getQueueProperties(ThreeListenersBeans.QUEUE_1)).isNull();
                    assertThat(rabbitAdmin2.getQueueProperties(ThreeListenersBeans.QUEUE_2)).isNotNull();
                });
    }
